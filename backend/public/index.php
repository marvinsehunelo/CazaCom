...
<style>
/* Mobile Money Dashboard - Clean, Dark, Focused */
body { 
...
/* Mobile Money Theme - Orange/Amber Accent */
.btn-tab { 
    background: #1a1a1a; 
    color: #fff; 
    font-weight: 500; 
...
/* Mobile Money Balance Card Widget */
.balance-widget {
    background-color: #1a1a1a;
    border: 1px solid #1f1f1f;
...
<div class="portal-container p-8 sm:p-12">
    ...
    <!-- Navigation Tabs - Only Mobile Money -->
...
async function loadMobileMoneyData() {
    try {
        ...
        // Load mobile money transfers
        if (historyRes.transfers) {
            displayMobileMoneyTransfers(historyRes.transfers || []);
        }
        
        // Load airtime purchases
        if (historyRes.airtime) {
            displayAirtimePurchases(historyRes.airtime || []);
        }
    } catch (error) {
...
function displayMobileMoneyTransfers(transfers) {
    const tbody = document.getElementById('mobileMoneyTransfersList');
    if (!transfers || !transfers.length) {
        tbody.innerHTML = '<tr><td colspan="6" class="text-center p-4 text-gray-500">No transfers found</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = transfers.map(tx => `
        <tr class="border-b border-gray-800">
            <td class="p-2">${new Date(tx.created_at).toLocaleString()}</td>
            <td class="p-2">${tx.recipient_phone || '-'}</td>
            <td class="p-2">BWP ${parseFloat(tx.amount).toFixed(2)}</td>
            <td class="p-2">BWP ${parseFloat(tx.fee || 0).toFixed(2)}</td>
            <td class="p-2">${tx.network || '-'}</td>
            <td class="p-2"><span class="px-2 py-1 text-xs ${tx.status === 'completed' ? 'bg-green-900 text-green-400' : 'bg-yellow-900 text-yellow-400'}">${tx.status}</span></td>
        </tr>
    `).join('');
}

function displayAirtimePurchases(purchases) {
    const tbody = document.getElementById('airtimePurchasesList');
    if (!purchases || !purchases.length) {
        tbody.innerHTML = '<tr><td colspan="5" class="text-center p-4 text-gray-500">No purchases found</td></tr>';
        return;
    }
    
    tbody.innerHTML = purchases.map(tx => `
        <tr class="border-b border-gray-800">
            <td class="p-2">${new Date(tx.created_at).toLocaleString()}</td>
            <td class="p-2">${tx.phone || '-'}</td>
            <td class="p-2">BWP ${parseFloat(tx.amount).toFixed(2)}</td>
            <td class="p-2">${tx.network || '-'}</td>
            <td class="p-2"><span class="px-2 py-1 text-xs ${tx.status === 'completed' ? 'bg-green-900 text-green-400' : 'bg-yellow-900 text-yellow-400'}">${tx.status}</span></td>
        </tr>
