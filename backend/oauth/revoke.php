...
try {
    if ($tokenTypeHint === 'access_token') {
        $stmt = $db->prepare("UPDATE oauth_access_tokens SET revoked = TRUE, revoked_at = NOW() WHERE token = :token");
        $stmt->execute(['token' => $token]);
        
        // Also revoke associated refresh tokens
        $stmt = $db->prepare("UPDATE oauth_refresh_tokens SET revoked = TRUE, revoked_at = NOW() WHERE access_token = :token");
        $stmt->execute(['token' => $token]);
        
    } elseif ($tokenTypeHint === 'refresh_token') {
        $stmt = $db->prepare("UPDATE oauth_refresh_tokens SET revoked = TRUE, revoked_at = NOW() WHERE token = :token");
        $stmt->execute(['token' => $token]);
